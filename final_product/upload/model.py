...
class CURLLoss(nn.Module):
    ...
    def compute_ssim(self, img1, img2):
        ...
        ssim_map2 = ((mu1_sq.to("cpu") + mu2_sq.to("cpu") + C1) *
                     (sigma1_sq.to("cpu") + sigma2_sq.to("cpu") + C2))
        ssim_map = ssim_map1.to("cpu") / ssim_map2.to("cpu")

        v1 = 2.0 * sigma12.to("cpu") + C2
        v2 = sigma1_sq.to("cpu") + sigma2_sq.to("cpu") + C2
        cs = torch.mean(v1 / v2)
    ...
    def forward(self, predicted_img_batch, target_img_batch, gradient_regulariser):
        ...
        ssim_loss_value = Variable(
            torch.FloatTensor(torch.zeros(1, 1).to("cpu")))
        ...
        for i in range(0, num_images):
            ...
            target_img = target_img_batch[i, :, :, :].to("cpu")
            predicted_img = predicted_img_batch[i, :, :, :].to("cpu")
