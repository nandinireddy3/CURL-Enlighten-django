    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            if idx in self.data_dict:
                ...
                input_img = np.load(self.data_dict[idx]['input_img'], map_location='cpu')
...
########################################################################################################        
        ...
        for file in os.scandir(self.data_dirpath):
            if file.is_file():
                ...
                if is_id_in_list:  # check that the image is a member of the appropriate training/test/validation split
                    ...
                    if not img_id in img_id_to_idx_dict.keys():
                      ...
                      # change this to the name of your
                                        # input data folder

                    input_img_filepath = file.name

                    self.data_dict[idx_tmp]['input_img'] = self.data_dirpath + \
                            "/" + input_img_filepath
                            
                    output_img_filepath = file.name

                    self.data_dict[idx_tmp]['output_img'] = self.data_dirpath + \
                        "/" + output_img_filepath
