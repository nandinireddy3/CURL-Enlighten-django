...
class ImageProcessing(object):
    ...
    def rgb_to_lab(img, is_training=True):
        ...
        img = torch.mul(img, Variable(torch.FloatTensor(
            [1/0.950456, 1.0, 1/1.088754]), requires_grad=False).to("cpu"))
        ...
        img = torch.matmul(img, fxfyfz_to_lab) + Variable(
            torch.FloatTensor([-16.0, 0.0, 0.0]), requires_grad=False).to("cpu")
    ...
    def rgb_to_hsv(img):
        ...
        ones = Variable(torch.FloatTensor(
            torch.ones((img.shape[0])))).to("cpu")
        zero = Variable(torch.FloatTensor(torch.zeros(shape[0:2]))).to("cpu")
        ...
        g = img[:, :, 1].clone().to("cpu")
        b = img[:, :, 2].clone().to("cpu")
        r = img[:, :, 0].clone().to("cpu")
    ...
    clamp=False):
        ...
        """
        slope = Variable(torch.zeros((C.shape[0]-1))).to("cpu")
    ...
    def adjust_hsv(img, S):
        ...
        slope_sqr_diff = Variable(torch.zeros(1)*0.0).to("cpu")
