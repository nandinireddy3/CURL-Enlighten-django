...
class Evaluator():
    ...
    def __init__(self, criterion, data_loader, split_name, out_path):
    ...
    def evaluate(self, net, epoch=0):
        ...
        with torch.no_grad():
            ...
            for batch_num, data in enumerate(self.data_loader, 0):
                ...
                input_img_batch, output_img_batch, name = Variable(data['input_img'], requires_grad=False).to("cpu"), Variable(data['output_img'],
                                                                                                   requires_grad=False).to("cpu"), data['name']
                ...
                for i in range(0, input_img_batch.shape[0]):
                    ...
                    # print(type(net_output_img_example_rgb))
                    # print(net_output_img_example_rgb.shape)
                    # img = Image.fromarray(net_output_img_example_rgb[0], 'RGB')
                    # img.save('my.png')
                    # img.show()
                    ...
                    if batch_num > 30:
                        ...
                        plt.imsave(out_dirpath + "/" + name[0].split(".")[0] + ".jpg",
